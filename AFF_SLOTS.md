...
- 2026-09-23｜実数 18／表 18｜差分なし（`data-aff` の grep は**17行**＝静的16（courses9・revenge5・kaitou1・chinkan1）＋ `index.html` の `adCard()` テンプレ1本が `promoHome`/`promoResult` の2枠を生成。前回の予告どおり「grep 17行＝18枠」で数えられた＝worktree 除去の効果が持続している）
